...
public class GvrPointerInputModuleImpl {
  ...
  public void Process() {
    ...
    // Handle input
		if (!triggerDown && triggering || (Input.GetKeyDown("joystick button 0") && Input.GetKey("joystick button 0"))) {
      HandleDrag();
		} else if (triggerDown && !CurrentEventData.eligibleForClick || Input.GetKeyDown("joystick button 0")) {
      // New trigger action.
      HandleTriggerDown();
